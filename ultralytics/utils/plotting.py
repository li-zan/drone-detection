...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        non_ascii = not is_ascii(example)  # 检查是否含有非ASCII字符，比如中文
        ...
        if self.pil:  # 使用 PIL 绘图
            self.im = im if input_is_pil else Image.fromarray(im)
            self.draw = ImageDraw.Draw(self.im)
            if non_ascii:
                font = check_font("simsun.ttc")  # 使用包含中文的字体
            else:
        ...
        # print(f"使用PIL: {self.pil}, Draw初始化: {'draw' in dir(self)}")
    ...
    def statistics(self, statistics):
        ...
        font = ImageFont.truetype(
            "simsun.ttc", 40, encoding="utf-8")
        # 绘制黄底
        self.im = cv2.cvtColor(np.asarray(self.im), cv2.COLOR_RGB2BGR)
        blk = np.zeros(self.im.shape, np.uint8)
        cv2.rectangle(blk, (left - 10, top - 10), (right, top + length * stride + 10), color=(255, 255, 0),
                      thickness=-1)
        self.im = cv2.addWeighted(self.im, 1.0, blk, 0.7, 1)
        self.im = Image.fromarray(cv2.cvtColor(self.im, cv2.COLOR_BGR2RGB))
        self.draw = ImageDraw.Draw(self.im)
        # self.draw.rectangle(((left - 10, top - 10), (right, top + length * stride + 10)), fill=(0, 255, 255))
        # 写入统计数据
        for key, values in statistics.items():
            self.draw.text((left, top), key + ":" + str(values), font=font, fill=(0, 0, 255))
            top += stride
