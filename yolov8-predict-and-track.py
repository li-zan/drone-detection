...
# ----------------路径设置----------------------------------
source_path = "video/test1.mp4"  # 视频路径
weights_path = "weights/yolov8-1280.pt"  # 权重文件路径
...
for r in results:
    ...
    if r.boxes.id is not None:  # boxes.id:盒子的轨道id(如果可以被跟踪算法检测的话就为True)。
        ...
        if person_change_count >= person_increase_or_decrease:
            # ----------------------处理选框------------------------
            out_img = test_corner_boxes(im_array, boxes.xywh, boxes.xyxy, boxes.cls, boxes.id, l=20,
                                        is_transparent=True,
            ...
            # -----------------------------------------------------
            person_change_count = 0
            cap_alarm(file_path, num_person, num_helmet)
