# track by yolo_model   ---------------   yolo模型预测+追踪

import datetime
import os

from ClassCountFormatter import generate_detailed_statistics_string
...
from vehicle_alarm import vehicle_alarm
...
# ----------------路径设置----------------------------------
source_path = "E:/videos_for_test/DJI_20240416155626_0011_V.mp4"  # 视频路径
...
for r in results:
    ...
    if r.boxes.id is not None:  # boxes.id:盒子的轨道id(如果可以被跟踪算法检测的话就为True)。
        ...
        if time.time() - p_start >= 6:
            ...
            # vehicle_alarm(_file_path, count_per_class)
            p_start = time.time()
            # print("6秒时间到")
        # ----------------------------------------------------------------
...
# ---------------------------------------------------------------
print(objects_location_string)
# vehicle_alarm(_file_path, count_per_class, UAV_status, class_ids_string, objects_location_string)
# ----------------------------------------------------------------
