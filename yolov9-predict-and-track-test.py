...
from get_UAV_status49 import get_UAV_status
...
def detect(UAV_status):
    # ----------------路径设置----------------------------------
    source_path = "E:/videos_for_test/prj49/target2.mp4"  # 视频路径
    ...
    rtsp_url = 'rtsp://10.14.197.102:8554/' # RTSP服务器地址
    ...
    for r in results:
        ...
        if r.boxes.id is not None:  # boxes.id:盒子的轨道id(如果可以被跟踪算法检测的话就为True)。
            ...
            if time.time() - p_start >= 30:
                ...
                # vehicle_alarm(_file_path, count_per_class, UAV_status, class_ids_vehicle_string, objects_location_string)
                p_start = time.time()
                # print("6秒时间到")
...
if __name__ == "__main__":
    UAV_status = {
        "data": {
            ...
            "uavRecordId": "test",
            "projectId": "test",
            "rtmpSubUrl": "E:/videos_for_test/prj49/target2.mp4"
        }
    }
    detect(UAV_status)
# -----------------------------------------------------------------------------------------------------------------------------
